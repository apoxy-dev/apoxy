...
func (m *ApoxyCli) PublishGithubRelease(
	...
	return dag.Container().
		...
		// feed the brew formula; we also emit goarch-named (amd64) copies for
		// `apoxy upgrade` (getsavvyinc/upgrade-cli) to match against. The arm64
		// Homebrew names already match the upgrader's (case-insensitive) matcher,
		// so we don't duplicate them — GitHub asset names are case-insensitive
		// for uniqueness and `apoxy_Linux_arm64` / `apoxy_linux_arm64` would collide.
...
func (m *ApoxyCli) PublishEdgeRuntime(
	...
	builder := m.BuildEdgeRuntime(ctx, platform, nil).
...
func (m *ApoxyCli) BuildAPIServer(
	...
	runtimeCtr := m.PullEdgeRuntime(ctx, p, src, edgeRuntimeTag, gcrCreds)
...
// buildWorkerdManager builds the workerd-manager runtime container: the APO-796
// ServiceManager data plane. It is its own runsc re-exec target
// (sandbox.DispatchRunsc) and runs stock workerd in a gVisor sandbox. It is not a
// standalone dagger function (no dagger.gen.go case) — the publish loop calls it
// directly — because regenerating the dagger bindings is out of band; promote it
// to an exported dagger function via codegen when convenient.
func (m *ApoxyCli) buildWorkerdManager(
	ctx context.Context,
	src *dagger.Directory,
	platform string,
	...
	return m.buildRunscBinaryImage(ctx, src, platform, clrkSrc, githubToken, "./cmd/workerd-manager", "workerd-manager")
}

// buildRunscBinaryImage builds a static, runsc-capable container for the
// workerd-manager DispatchRunsc binary. The binary is its own runsc re-exec
// target, so the image entrypoint and /proc/self/exe must be the same stable
// path. CGO is off (the gVisor systrap runtime + in-Sentry netstack link without
// cgo), matching the GOOS=linux cross-compile.
//
// clrk is a private module. For local builds, pass clrkSrc to mount a sibling
// clrk checkout and replace the go.mod pin. For CI, pass githubToken so the
// builder can fetch the pinned clrk commit over HTTPS.
func (m *ApoxyCli) buildRunscBinaryImage(
	ctx context.Context,
	src *dagger.Directory,
	platform string,
	clrkSrc *dagger.Directory,
	...
	out := filepath.Join("build", binName+"-"+goarch)
	...
	// builds, or iterating on clrk alongside the runtime).
	if clrkSrc != nil {
		builder = builder.
	...
	builder = builder.
		WithExec([]string{"go", "build", "-ldflags", "-v", "-o", out, cmdPath}).
		WithWorkdir("/src")

	return dag.Container(dagger.ContainerOpts{Platform: p}).
		From("cgr.dev/chainguard/wolfi-base:latest").
		// runsc needs a real rootfs (the Sentry chroot reads /etc/localtime) plus
		// basic net tooling for sandbox setup and the resident egress path.
		WithExec([]string{"apk", "add", "-u", "iptables", "ip6tables", "iproute2"}).
		WithFile("/bin/"+binName, builder.File(out)).
		WithEntrypoint([]string{"/bin/" + binName})
}

// buildWorkerd builds the stock upstream workerd OCI image the APO-796 resident
// runs (referenced via `apoxy dev --workerd-image` and the manager's
// --workerd_image). It does NOT compile workerd: it lifts the prebuilt
// @cloudflare/workerd-linux-<arch> binary from npm into a minimal wolfi image
// with `workerd` on PATH. The npm package name encodes the target arch, so the
// fetch container's own platform is irrelevant. Pin `version` for reproducibility;
// it must support the WorkerLoader binding the resident dispatcher uses.
func (m *ApoxyCli) buildWorkerd(
	ctx context.Context,
	platform string,
	// +optional
	version string,
	...
	npmPkg := "@cloudflare/workerd-linux-" + npmArch
	...
	return dag.Container(dagger.ContainerOpts{Platform: p}).
		From("cgr.dev/chainguard/wolfi-base:latest").
		// workerd is dynamically linked against libstdc++/libgcc.
		WithExec([]string{"apk", "add", "-u", "libstdc++", "libgcc"}).
		WithFile("/usr/bin/workerd", workerdBin).
...
func (m *ApoxyCli) PublishImages(
	...
	for _, platform := range linuxPlatforms {
		apiCtrs = append(apiCtrs, m.BuildAPIServer(ctx, src, platform, edgeRuntimeTag, gcrCreds))
	}
	...
	for _, platform := range linuxPlatforms {
		bCtrs = append(bCtrs, m.BuildBackplane(ctx, src, platform, edgeRuntimeTag, gcrCreds))
	}
	...
	// The stock workerd runtime image builds from the npm prebuilt with no
	// clrk/source dependency, so it always publishes.
	...
	// workerd-manager depends on the private clrk module, which the default CI
	// GITHUB_TOKEN cannot fetch (clrk is a separate private repo). Publish it only
	// when a clrk-capable token is supplied.
	if githubToken != nil {
		...
		fmt.Println("Skipping workerd-manager publish: no --github-token for the private clrk module")
	}
