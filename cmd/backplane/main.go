...
var (
	...
	envoyReleaseURL = flag.String("envoy_release_url", "", "URL to the Envoy release binary. If a \"<URL>.sha256\" file is published, the download is checked against it.")
	envoyVersion    = flag.String("envoy_version", envoy.DefaultVersion, "Envoy release tag to download from GitHub, for example v1.35.13. An empty value selects the latest upstream release, which must not be used in production. The Proxy spec and --envoy_release_url take precedence.")
