...
type DomainZoneLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.DomainZone, err error)
	// Get retrieves the DomainZone from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.DomainZone, error)
...
// List lists all DomainZones in the indexer.
func (s *domainZoneLister) List(selector labels.Selector) (ret []*v1alpha2.DomainZone, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.DomainZone))
	})
	return ret, err
}

// Get retrieves the DomainZone from the index for a given name.
func (s *domainZoneLister) Get(name string) (*v1alpha2.DomainZone, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("domainzone"), name)
	}
	return obj.(*v1alpha2.DomainZone), nil
}
