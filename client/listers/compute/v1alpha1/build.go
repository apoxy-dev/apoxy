...
type BuildLister interface {
	...
	List(selector labels.Selector) (ret []*computev1alpha1.Build, err error)
	// Get retrieves the Build from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*computev1alpha1.Build, error)
	BuildListerExpansion
}

// buildLister implements the BuildLister interface.
type buildLister struct {
	listers.ResourceIndexer[*computev1alpha1.Build]
}

// NewBuildLister returns a new BuildLister.
func NewBuildLister(indexer cache.Indexer) BuildLister {
	return &buildLister{listers.New[*computev1alpha1.Build](indexer, computev1alpha1.Resource("build"))}
}
