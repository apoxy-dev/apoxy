...
type EgressGatewayLister interface {
	...
	List(selector labels.Selector) (ret []*computev1alpha1.EgressGateway, err error)
	// Get retrieves the EgressGateway from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*computev1alpha1.EgressGateway, error)
...
type egressGatewayLister struct {
	listers.ResourceIndexer[*computev1alpha1.EgressGateway]
}
...
func NewEgressGatewayLister(indexer cache.Indexer) EgressGatewayLister {
	return &egressGatewayLister{listers.New[*computev1alpha1.EgressGateway](indexer, computev1alpha1.Resource("egressgateway"))}
}
