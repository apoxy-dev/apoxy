...
type ServiceLister interface {
	...
	List(selector labels.Selector) (ret []*computev1alpha1.Service, err error)
	// Get retrieves the Service from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*computev1alpha1.Service, error)
	ServiceListerExpansion
}

// serviceLister implements the ServiceLister interface.
type serviceLister struct {
	listers.ResourceIndexer[*computev1alpha1.Service]
}

// NewServiceLister returns a new ServiceLister.
func NewServiceLister(indexer cache.Indexer) ServiceLister {
	return &serviceLister{listers.New[*computev1alpha1.Service](indexer, computev1alpha1.Resource("service"))}
}
