...
type EdgeFunctionRevisionLister interface {
	...
	List(selector labels.Selector) (ret []*extensionsv1alpha2.EdgeFunctionRevision, err error)
	// Get retrieves the EdgeFunctionRevision from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*extensionsv1alpha2.EdgeFunctionRevision, error)
...
type edgeFunctionRevisionLister struct {
	listers.ResourceIndexer[*extensionsv1alpha2.EdgeFunctionRevision]
}
...
func NewEdgeFunctionRevisionLister(indexer cache.Indexer) EdgeFunctionRevisionLister {
	return &edgeFunctionRevisionLister{listers.New[*extensionsv1alpha2.EdgeFunctionRevision](indexer, extensionsv1alpha2.Resource("edgefunctionrevision"))}
}
