...
type EdgeFunctionLister interface {
	...
	List(selector labels.Selector) (ret []*extensionsv1alpha1.EdgeFunction, err error)
	// Get retrieves the EdgeFunction from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*extensionsv1alpha1.EdgeFunction, error)
	EdgeFunctionListerExpansion
}

// edgeFunctionLister implements the EdgeFunctionLister interface.
type edgeFunctionLister struct {
	listers.ResourceIndexer[*extensionsv1alpha1.EdgeFunction]
}
...
func NewEdgeFunctionLister(indexer cache.Indexer) EdgeFunctionLister {
	return &edgeFunctionLister{listers.New[*extensionsv1alpha1.EdgeFunction](indexer, extensionsv1alpha1.Resource("edgefunction"))}
}
