...
type TLSRouteLister interface {
	...
	List(selector labels.Selector) (ret []*gatewayv1alpha2.TLSRoute, err error)
	// Get retrieves the TLSRoute from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*gatewayv1alpha2.TLSRoute, error)
	TLSRouteListerExpansion
}

// tLSRouteLister implements the TLSRouteLister interface.
type tLSRouteLister struct {
	listers.ResourceIndexer[*gatewayv1alpha2.TLSRoute]
}

// NewTLSRouteLister returns a new TLSRouteLister.
func NewTLSRouteLister(indexer cache.Indexer) TLSRouteLister {
	return &tLSRouteLister{listers.New[*gatewayv1alpha2.TLSRoute](indexer, gatewayv1alpha2.Resource("tlsroute"))}
}
