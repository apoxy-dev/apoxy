...
func TestICXNetwork_Speed(t *testing.T) {
	...
	hA, err := icx.NewHandler(icx.WithLocalAddr(netstack.ToFullAddress(uaA)),
		icx.WithVirtMAC(tcpip.GetRandMacAddr()), icx.WithLayer3VirtFrames())
	require.NoError(t, err)

	hB, err := icx.NewHandler(icx.WithLocalAddr(netstack.ToFullAddress(uaB)),
		icx.WithVirtMAC(tcpip.GetRandMacAddr()), icx.WithLayer3VirtFrames())
	require.NoError(t, err)
	...
	err = hA.AddVirtualNetwork(vni, netstack.ToFullAddress(uaB), []netip.Prefix{route})
	require.NoError(t, err)

	err = hB.AddVirtualNetwork(vni, netstack.ToFullAddress(uaA), []netip.Prefix{route})
	require.NoError(t, err)
