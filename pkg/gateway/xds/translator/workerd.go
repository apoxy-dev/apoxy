...
// x-apoxy-service demux header with the bare service name. It is stateless and
// carries no project: the resident is per-tenant, the only runtime fact it needs
// (the resident socket) is a fixed well-known path, and the resident's dispatcher
// owns all revision and liveness demux via /resolve, so a rollout never
// re-translates xDS. It is a no-op when no route is workerd-backed, so it is safe
// to register unconditionally — the shared backplane runs the same in-tree filter
// chain and picks it up automatically.
//
// INVARIANT (load-bearing): a workerd resident serves exactly one project. The
// bare-service key is unambiguous only because of this — one resident per
// backplane, scoped to a single project's apiserver (the manager's kube client),
// reached over one fixed socket. The shared backplane's in-tree hook fires too,
// but no multi-project resident sits behind it today (there is no workerd-manager
// in the shared-backplane deployment, and route synthesis from a compute Service
// is unbuilt — APO-628). If a single resident is ever fronted by a multi-tenant
// backplane for many projects, two projects' same-named Services would collide on
// the bare key and the project qualifier (or per-project residents) must return.
type workerd struct{}
...
// the x-apoxy-service demux header to the bare service name (the resident's
// dispatcher resolves the live revision). It is a no-op for non-workerd routes
// and for redirect/direct-response routes (which have no RouteAction to re-point).
func (*workerd) patchRoute(route *routev3.Route, irRoute *ir.HTTPRoute) error {
	...
	log.Infof("Demuxed route %s to workerd resident (%s=%s)", route.GetName(), workerdServiceHeader, irRoute.WorkerdService)
	return nil
}
